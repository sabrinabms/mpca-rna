...
PROGRAM MPCA
    ...
    !comentado por Sabrina em 30/01/17
...
!        tryFixedConfiguration
    ...
    !Allocating space for dynamic input variables
    allocate(op % x(op % nInputs, op % nClasses))
    allocate(op % y(op % nOutputs, op % nClasses))

    !Reading the ANN input and output data
    fString = '(      F13.8)'
    write(fString(2:7), '(I6)') op % nClasses

    OPEN (2, file = './data/perfilVertTU/ComRuido/x.txt')
    DO i = 1, op % nInputs
...
!        read(2, fString) (op % x(i, j), j = 1, op % nClasses)
    ...
    OPEN (1, file = './data/perfilVertTU/ComRuido/yd.txt')
    DO I = 1, op % nOutputs
...
!        read(1, fString) (op % y(i, j), j = 1, op % nClasses)
    ...
    if (op % haveValidation .eqv. .true.) then
        ...
        OPEN (1, file = './data/perfilVertTU/ComRuido/yd_valid.txt')
        DO i = 1, op % nOutputs
            read(1, *) (op % y_valid(i, j), j = 1, op % nClassesValidation)
        END DO
        CLOSE (1)

        OPEN (2, file = './data/perfilVertTU/ComRuido/x_valid.txt')
        DO i = 1, op % nInputs
    ...
    DO contP = 1, op % nParticlesProcessor
        ...
        !else if (contP == 1 .and. tryFixedConfiguration .eqv. .true.) then
        !    oldParticle(contP) % solution(1) = initial_Hidden_Layers
        !    oldParticle(contP) % solution(2) = initial_First_Hidden_Layer
        !    oldParticle(contP) % solution(3) = initial_Second_Hidden_Layer
        else
        ...
        oldParticle(contP) % fitness = neuralNetworkTrainning(oldParticle(contP) % solution, op, st)
        st % NFE = st % NFE + 1
    ...
    if (op % iProcessor == 0) then    
        OPEN(UNIT = 20, FILE = './data/perfilVertTU/ComRuido/final.out', ACCESS = 'APPEND')
