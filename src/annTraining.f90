...
CONTAINS
    ...
    REAL(kind = 8) FUNCTION neuralNetworkTrainning(solution, op, st)
        ...
        integer :: k
        integer :: epoch
        integer :: loadWeightsProcessor

        config % hiddenLayers = nint(solution(1))
        config % neuronsLayer(1) = nint(solution(2))
        config % neuronsLayer(2) = nint(solution(3))
        config % activationFunction = nint(solution(4))
        config % alpha = solution(5)
        config % eta = solution(6)
       
        !Reading initial parameters ANN 
        if(op % loadWeightsBias == 2 .and. op % iProcessor == 0) then
            loadWeightsProcessor = 2
        ...
        neuralNetworkTrainning = 0
        
        !--------------------------------------------------------------------!
        !INITIAL WEIGHTS AND BIASES
        !--------------------------------------------------------------------!

        SELECT CASE (loadWeightsProcessor)
        ...
        IF (op % haveValidation .eqv. .true.) THEN
            neuralNetworkTrainning = penaltyObj &
                & * ((alphaObj * config % MeanSquaredError + betaObj * config % MeanSquaredErrorValidation) &
                & / (alphaObj + betaObj))
        ELSE
            neuralNetworkTrainning = penaltyObj * config % MeanSquaredError
        ENDIF

        ! Store configuration if objFunction is best
        if (neuralNetworkTrainning < st % bestObjectiveFunction) then
            
            
            st % bestObjectiveFunction = neuralNetworkTrainning
    ...
    END FUNCTION neuralNetworkTrainning
