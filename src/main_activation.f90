PROGRAM MAIN_ACTIVATION
    ...
    !Reading input data
    OPEN (2, file = './data/perfilVertTU/ComRuido/x_gen.txt')
    open(3, file='./data/perfilVertTU/ComRuido/x_gen_lido.txt')
    DO i = 1, op % nInputs
        READ(2, *) (op % x(i, j), j = 1, op % nClasses)
        write(3,*) (op % x(i, j), j = 1, op % nClasses)
    END DO
    CLOSE (2)
    
    OPEN (2, file = './data/perfilVertTU/ComRuido/yd_gen.txt')
    DO i = 1, op % nInputs
        READ(2, *) (op % y(i, j), j = 1, op % nClasses)
    END DO
    CLOSE (2)
    ...
    open(12, file = trim('./output/best.out'), STATUS = "old")
    read(12, '(A)') dummy
